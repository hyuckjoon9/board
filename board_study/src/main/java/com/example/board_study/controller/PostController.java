...
public class PostController {
    ...
    public void create(@RequestBody PostCreateRequest request) {
        postService.create(request.getTitle(), request.getContent(), getCurrentUsername());
    }
    ...
    public void update(
        ...
        postService.update(id, request.getTitle(), request.getContent(), getCurrentUsername());
    }
    ...
    public void delete(
            @Parameter(description = "게시글 ID", example = "1")
            @PathVariable Long id) {
        postService.delete(id, getCurrentUsername());
    }
}
